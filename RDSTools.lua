...
local cfg = inicfg.load({ -- áàçîâûå íàñòðîéêè ñêðèïòà
	...
	script = {
		version = 0.1,
		info = 'Èçìåíåíû òåêñòäðàâû\nîáíîâë¸í èíòåðôåéñ\nÄîáàâëåí àâòîìóò\nÓäà÷íîãî èñïîëüçîâàíèÿ'
	}
}, directIni)
...
imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\FRAMDIT.ttf', 17, nil, glyph_ranges) -- øðèôò èíòåðôåéñà
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(RDSTools.script.version) > version then
                update_state = true
				sampAddChatMessage('RDS Tools: Íàéäåíî îáíîâëåíèå, ïðîâåðèòü ÷òî äîáàâëåíî êîìàíäîé /check_update, çàãðóæàþ ... ', -1)
			else
				sampAddChatMessage('Ñêðèïò {FF0000}RDS Tools ' .. '{C0C0C0}[' .. version .. ']' ..  ' {FFFFFF}çàãðóæåí, àêòèâàöèÿ: {808080}F3', -1)
			end
...
function imgui.OnDrawFrame()
	...
	if imgui.Checkbox(u8'Ïðèâåòñòâèå àäìèíèñòðàòîðîâ', checked_test2) then
	...
	if imgui.Checkbox(u8"Chat Cleaner", checked_test5) then
		...
		inicfg.save(cfg, directIni)
		sampShowDialog(1000, "Èíôîðìàöèÿ", "Chat Cleaner äëÿ âêë/âûêë òðåáóåò ïåðåçàãðóçêó èãðû. Àêòèâàöèÿ: /cleaner", "Ïîíÿë", _)
		showCursor(false,false)
		thisScript():reload()
	end
		imgui.SameLine()
	...
	if imgui.Checkbox(u8"Fast Spawn", checked_test6) then
		...
		inicfg.save(cfg, directIni)
		sampShowDialog(1000, "Èíôîðìàöèÿ", "Fast spawn äëÿ âêë/âûêë òðåáóåò ïåðåçàãðóçêó èãðû. Àêòèâàöèÿ: /fs", "Ïîíÿë", _)
		showCursor(false,false)
		thisScript():reload()
	end
	if imgui.Checkbox(u8"Ñâîáîäíî", checked_test7) then
		cfg.settings.FLD = not cfg.settings.FLD
		inicfg.save(cfg, directIni)
	end
	imgui.SameLine()
	imgui.SetCursorPosX(225)
	if imgui.Checkbox(u8"Ñâîáîäíî", checked_test8) then
		cfg.settings.clickwarp = not cfg.settings.clickwarp
		inicfg.save(cfg, directIni)
	end
	if imgui.Checkbox(u8"Ñâîáîäíî", checked_test9) then
...
/newprfma - èçìåíèòü öâåò ïðåôèêñà ÌÀ\n/newprfa - èçìåíèòü öâåò ïðåôèêñà À\n/newprfsa - èçìåíèòü öâåò ïðåôèêñà ÑÀ\n/newprfnick - èçìåíèòü äîëæíîñòü (äëÿ ðàíäîì ïðåôèêñà)', "Ñïàñèáî", "", 0)
	end
	imgui.End()
	end
	if secondary_window_state.v then -- âòîðîå îêíî íà F2
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		...
		imgui.Text(u8"Òóò áóäåò ðåêîí ìåíþ")
		imgui.End()
	end
end
...
sampRegisterChatCommand('ror', function(param) 
	sampSendChat('/rmute ' .. param .. " 2500 Îñêîðáëåíèå/Óïîìèíàíèå Ðîäíè")
end)
