    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(RDSToolsLite.script.version) > version then
                update_state = true
				sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Íàéäåíî îáíîâëåíèå, çàãðóçèòüñÿ ìîæíî â îñíîâíîì ìåíþ, àêòèâàöèÿ: {808080}F3 ', -1)
			end
	...
	sampRegisterChatCommand('update', function(param)
		if update_state then
			downloadUrlToFile(script_url, script_path, function(id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					wait(10000)
					sampShowDialog(1000, "xX RDS Tools Xx", '{FFFFFF}Áûëà íàéäåíà íîâàÿ âåðñèÿ - ' .. RDSToolsLite.script.version .. '\n{FFFFFF}Â íåé äîáàâëåíî ' .. RDSTools.script.info, "Ñïàñèáî", "", 0)
					showCursor(false,false)
					thisScript():reload()
				end
			end)
		else
			sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Ó âàñ óñòàíîâëåíà àêòóàëüíàÿ âåðñèÿ.')
		end
	end)
	imgui.Process = false
...
function imgui.OnDrawFrame()
	...
	if main_window_state.v then -- ÊÍÎÏÊÈ ÈÍÒÅÐÔÅÉÑÀ F3
		...
		if imgui.Button(u8"Âêë/âûêë", imgui.ImVec2(125, 20)) then uu2() menu2[1] = true end imgui.SameLine()
        if imgui.Button(u8'Äîïîëíåíèå', imgui.ImVec2(125, 20)) then uu2() menu2[2] = true end 
		...
		if menu2[1] then
			...
			if imgui.Checkbox(u8"Fast Spawn", checked_test6) then
				if cfg.settings.fastspawn then
					...
					cfg.settings.fastspawn = not cfg.settings.fastspawn
					inicfg.save(cfg, directIni)
					sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}: Çà÷åì ïàðèòüñÿ ïðè âõîäå? Âñ¸ ñäåëàþ ñàì, òû òîëüêî íàñòðîé. Àêòèâàöèÿ: /fs', -1)
					showCursor(false,false)
					thisScript():reload()
				end
			end
			...
			if imgui.Checkbox(u8"Òðàññåðà", checked_test11) then
				if cfg.settings.trassera then
					...
					cfg.settings.trassera = not cfg.settings.trassera
					inicfg.save(cfg,directIni)
					sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}: Íè îäèí âûñòðåë íå ïðîéä¸ò íåçàìå÷åííûì! Àêòèâàöèÿ: /trassera')
					showCursor(false,false)
					thisScript():reload()
				end
			end
		...
		if menu2[2] then
			imgui.SetCursorPosX(10)
			if imgui.Button(u8'Îòêðûòü êîìàíäû', imgui.ImVec2(250, 24)) then
				secondary_window_state.v = not secondary_window_state.v
			end
			imgui.SetCursorPosX(10)
			if imgui.Button(u8'Îòêðûòü ôëóäû', imgui.ImVec2(250, 24)) then
				floods_window_state.v = not floods_window_state.v
			end
			imgui.SetCursorPosX(10)
			if imgui.Button(u8'Îòêðûòü áëîêíîò', imgui.ImVec2(250, 24)) then
				bloknotik_window_state.v = not bloknotik_window_state.v
			end
			imgui.SetCursorPosX(10)
			if imgui.Button(u8'Îòêðûòü áûñòðûå êëàâèøè', imgui.ImVec2(250, 24)) then
				fastcomand_window_state.v = not fastcomand_window_state.v
			end
			imgui.SetCursorPosX(10)
			imgui.Separator()
			if imgui.Button(u8'Ñîõðàíèòü ïîçèöèþ Recon Menu', imgui.ImVec2(250, 24)) then
				fourtwo_window_state.v = not fourtwo_window_state.v
			end
			imgui.SetCursorPosX(10)
			if cfg.settings.acon then
				if imgui.Button(u8'Íàñòðîéêà àäìèí-÷àòà', imgui.ImVec2(250, 24)) then
					ac_window_state.v = not ac_window_state.v
				end
			end
			imgui.SetCursorPosX(10)
			if cfg.settings.keysync then
				if imgui.Button(u8'Ñîõðàíèòü ïîçèöèþ âèðò.êëàâèø', imgui.ImVec2(250, 24)) then
	...
	if ac_window_state.v then
		...
		imgui.GetStyle().WindowTitleAlign = imgui.ImVec2(0.5, 0.5)
		imgui.GetStyle().ButtonTextAlign = imgui.ImVec2(0.5, 0.5)
		if imgui.Button(u8'Ñîõðàíèòü ïîçèöèþ') then
	...
	if fastcomand_window_state.v then -- áûñòðûå êëàâèøè
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8"Çàæìè è ñîõðàíè", fastcomand_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders)
		imgui.Text(u8'Îòêðûòèå ðåïîðòà:')
		imgui.SameLine()
		imgui.Text(u8(cfg.settings.ans))
		if imgui.Button(u8"Ñoxðaíèòü.", imgui.ImVec2(230, 0)) then
			cfg.settings.ans = getDownKeysText()
			inicfg.save(cfg,directIni)
		end
		imgui.Text(u8'Âêë/âûêë áûñòðîãî ðåïîðòà')
		imgui.SameLine()
		imgui.Text(u8(cfg.settings.tr))
		if imgui.Button(u8"Ñîxðaíèòü.", imgui.ImVec2(230, 0)) then
			cfg.settings.tr = getDownKeysText()
			inicfg.save(cfg,directIni)
		end
		imgui.Text(u8"Âêë/âûêë WallHack: ")
		imgui.SameLine()
		imgui.Text(u8(cfg.settings.wh))
		if imgui.Button(u8"Cîxðaíèòü.", imgui.ImVec2(230, 0)) then
			cfg.settings.wh = getDownKeysText()
			inicfg.save(cfg,directIni)
		end
		imgui.Text(u8"Âêë/âûêë áåññìåðòèÿ: ")
		imgui.SameLine()
		imgui.Text(u8(cfg.settings.agm))
		if imgui.Button(u8"Coxðaíèòü.", imgui.ImVec2(230, 0)) then
			cfg.settings.agm = getDownKeysText()
			inicfg.save(cfg,directIni)
		end
		imgui.Text(u8"Íàïîìèíàíèå â /a î ðåïîðòå: ")
		imgui.SameLine()
		imgui.Text(u8(cfg.settings.rep))
		if imgui.Button(u8"Ñîxpaíèòü.", imgui.ImVec2(230, 0)) then
			cfg.settings.rep = getDownKeysText()
			inicfg.save(cfg,directIni)
		end
		imgui.Separator()
		if imgui.Button(u8"Ñáðîñèòü âñå çíà÷åíèÿ.", imgui.ImVec2(230, 0)) then
	...
	if secondary_window_state.v then -- áûñòðûå êîìàíäû
		...
		imgui.CenterText(u8'Ìàêñèìàëüíîå íàêàçàíèå êîìàíä - 7 äíåé.')
	...
	if tree_window_state.v then -- áûñòðûé îòâåò íà ðåïîðò
		...
		if imgui.Button(u8'Íàêàçàòü àâòîðà', imgui.ImVec2(120, 25)) then
			nakajy = true
		end
		imgui.SameLine()
	...
	if bloknotik_window_state.v then -- áëîêíîò
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2)-200, (sh / 2)-250), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8"Áëîêíîò", bloknotik_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders)
	...
	if ban_window_state.v then
		...
		imgui.GetStyle().WindowTitleAlign = imgui.ImVec2(0.5, 0.5)
		imgui.GetStyle().ButtonTextAlign = imgui.ImVec2(0.5, 0.5)
		imgui.Text(u8'Âûáåðèòå ïðè÷èíó:')
		if not sampIsDialogActive() then
			...
			if imgui.Button(u8'Ìíîãî÷èñëåííûå íàðóøåíèÿ (3)', imgui.ImVec2(250, 25)) then
				sampSetChatInputText('/iban ' .. playerrecon .. ' 3 Íåàäåêâàòíîå ïîâåäåíèå')
				sampSetChatInputEnabled(true)
				setVirtualKeyDown(13, true)
				setVirtualKeyDown(13, false)
				showCursor(false,false)
			...
			if imgui.Button(u8'Íàçâàíèå áàíäû', imgui.ImVec2(250, 25)) then
				sampSendChat('/iban ' .. playerrecon .. ' 7 Íàçâàíèå áàíäû')
				showCursor(false,false)
				four_window_state.v = false
				ban_window_state.v = false
			end
		else
			sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: {FFFFFF}Çàêðîéòå äèàëîã, ÷òîáû ïðîäîëæèòü.', -1)
		end
		imgui.End()
	end
	...
	if mute_window_state.v then 
		...
		imgui.Text(u8'Âûáåðèòå ïðè÷èíó:')
		if not sampIsDialogActive() then
			if imgui.Button(u8'Îñêîðáëåíèå', imgui.ImVec2(250, 25)) then
			...
			if imgui.Button(u8'Îñêîðáëåíèå ðîäíûõ', imgui.ImVec2(250, 25)) then
	...
	if four_window_state.v then -- êàñòîì ðåêîí ìåíþ
		...
		imgui.GetStyle().ButtonTextAlign = imgui.ImVec2(0.5, 0.5)
		if imgui.Button(u8"Îñíîâíàÿ âêëàäêà", imgui.ImVec2(125, 20)) then uu() menu[1] = true end imgui.SameLine()
        if imgui.Button(u8'Èãðîêè â ðàäèóñå', imgui.ImVec2(125, 20)) then uu() menu[2] = true end 
        imgui.Separator()
        imgui.NewLine()
        imgui.SameLine(3)
        if menu[1] then
			...
			if imgui.Button(u8'+', imgui.ImVec2(20, 20)) then
				...
				sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: {FFFFFF}Íèê ñêîïèðîâàí â áóôôåð îáìåíà.', -1)
			end
			imgui.SameLine()
			...
			if imgui.Button(u8'Ïîñìîòðåòü ñòàòèñòèêó', imgui.ImVec2(250, 25)) then
				if not sampIsDialogActive() then
					...
					sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: {FFFFFF}Ó âàñ îòêðûò äèàëîã, çàêðîéòå åãî.', -1)
				end
			end
			if imgui.Button(u8'Ïîñìîòðåòü offline ñòàòèñòèêó', imgui.ImVec2(250, 25)) then
				if not sampIsDialogActive() then
					lua_thread.create(function()
			...
			if imgui.Button(u8'Îòêðûòü áûñòðûå êîìàíäû', imgui.ImVec2(250, 25)) then
	...
	if fourtwo_window_state.v then -- Ñîõðàíåíèÿ ðàñïîëîæåíèÿ êàñòîì ðåêîí ìåíþ
		...
		if imgui.Button(u8'Ñîõðàíèòü âûáðàííóþ ïîçèöèþ.', imgui.ImVec2(250, 25)) then
			if four_window_state.v then
				...
				sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: Çàéäèòå â ðåêîí çà êåì-íèáóäü âî èçáåæàíèÿ ðàññèõðîíà')
			end
		end
		imgui.End()
	end
	if five_window_state.v then -- Ñîõðàíåíèå ðàñïîëîæåíèÿ keylogger
		...
		if imgui.Button(u8"Ñîõðàíèòü ðàñïîëîæåíèå", imgui.ImVec2(250, 25)) then
			local pos = imgui.GetWindowPos()
	...
	if rmute_window_state.v then -- Íàêàçàòü â îêíå áûñòðîãî ðåïîðòà
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8"Âûäàòü áëîêèðîâêó ðåïîðòà", rmute_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders)
	...
	if floods_window_state.v then -- ôëóäû
		imgui.SetNextWindowSize(imgui.ImVec2(450, 325), imgui.Cond.FirstUseEver)
		imgui.SetNextWindowPos(imgui.ImVec2((sw / 2), sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(u8"Ôëóäû /mess", floods_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.ShowBorders)
		imgui.Columns(3)
			...
			if imgui.Button(u8'Î ôîðóìå è ãðóïïå', imgui.ImVec2(130, 25)) then
			...
			if imgui.Button(u8'Î /report', imgui.ImVec2(130, 25)) then
				...
				sampSendChat('/mess 13 Íàøåë ÷èòåðà, íàðóøèòåëÿ, ìàòåðøèíèêà èëè çëîñòíîãî ÄÌåðà?')
...
----- Ïîòîê äëÿ ðåíäåðà àäìèí ÷àòà ---------------
function sampev.onServerMessage(color,text) -- ïîèñê ñîîáùåíèé èç àäìèí-÷àòà
	if cfg.settings.acon then
		lua_thread.create(function()
			while true do
				wait(1)
	...
	if dialogId == 2349 then -- îêíî ñ ñàìèì ðåïîðòîì.
		...
		for line in text:gmatch("[^\n]+") do
			...
			if lineIndex == tonumber(1) - 1 then 
				...
				if don == '{' then
					autor = string.sub(autor, 24) -- ñ÷èòûâàåì àâòîðà æàëîáû
					autorid = sampGetPlayerIdByNickname(autor)
				end
			end
		end
		local lineIndex = -1
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED) -- óçíàåì ñâîé èä
		for line in text:gmatch("[^\n]+") do
			lineIndex = lineIndex + 1
			if lineIndex == tonumber(3) - 1 then 
				...
				don = string.sub(rev, -1)
				if don == '{' then
					textreport = string.sub(textreport, 9) -- òåêñò ðåïîðòà
		...
		lua_thread.create(function()
			while rabotay ~= 2 and uto4 ~= 2  and nakajy ~= 2 and slejy ~= 2 and jb ~= 2 and ojid ~= 2 and moiotvet ~= 2 and internet ~= 2 and uto4id ~= 2 and helpest~= 2 and nakazan ~= 2 and otklon ~= 2 and peredamrep ~= 2 do -- æäåì íàæàòèÿ êëàâèøè
				wait(50)
				...
				if nakajy then
					peremrep = ('Áóäåòå íàêàçàíû çà íàðóøåíèÿ ïðàâèë /report')
	...
	if dialogId == 2351 then -- îêíî ñ îòâåòîì íà ðåïîðò
		lua_thread.create(function()
			...
			if nakajy then
				...
				if oskrep then
					sampSendChat('/rmute ' .. autorid .. ' 400 Îñêîðáëåíèå')
					oskrep = false
				end
				if poprep then
				...
				if oskrod then
					sampSendChat('/rmute ' .. autorid .. ' 5000 Îñêîðáëåíèå/óïîìèíàíèå ðîäíè')
					oskrod = false
				end
				if capsrep then
...
function ev.onDisplayGameText(style, time, text) -- ñêðûâàåò òåêñò íà ýêðàíå.
...
sampRegisterChatCommand('wh' , function()
	if not cfg.settings.wallhack then
		...
		sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: {FFFFFF}WallHack âêëþ÷åí', -1)
		inicfg.save(cfg,directIni)
		local pStSet = sampGetServerSettingsPtr();
		NTdist = mem.getfloat(pStSet + 39)
		NTwalls = mem.getint8(pStSet + 47)
		...
		sampAddChatMessage('{FF0000}RDS Tools{d5d1eb}[' .. version .. ']: {FFFFFF}WallHack âûêëþ÷åí', -1)
		inicfg.save(cfg,directIni)
		local pStSet = sampGetServerSettingsPtr();
...
sampRegisterChatCommand('mytextreport', function(param)
	if #param ~= 0  then 
		...
		inicfg.save(cfg, directIni)
		sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Íîâûé äîïîëíèòåëüíûé òåêñò ïîñëå îòâåòà â ðåïîðò - ' .. param, -1)
	else
		sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Âû íå óêàçàëè çíà÷åíèå.')
	end
end)
...
sampRegisterChatCommand('n', function(param) 
	if #param ~= 0 then
		if cfg.settings.doptext then
			sampSendChat('/ans ' .. param .. ' Íå âèæó íàðóøåíèé ñî ñòîðîíû èãðîêà.' .. doptext)
		else
...
sampRegisterChatCommand('c', function(param) 
	if #param ~= 0 then
		if cfg.settings.doptext then
			sampSendChat('/ans ' .. param .. ' Íà÷àë(à) ðàáîòó íàä âàøåé æàëîáîé.' .. doptext)
		else
...
sampRegisterChatCommand('cl', function(param) 
	if #param ~= 0 then
		if cfg.settings.doptext then
			sampSendChat('/ans ' .. param .. ' Äàííûé èãðîê ÷èñò.' .. doptext)
		else
			sampSendChat('/ans ' .. param .. ' Äàííûé èãðîê ÷èñò.' .. doptext)
		end
	else
...
sampRegisterChatCommand('nv', function(param) 
	if #param ~= 0 then
		if cfg.settings.doptext then
			sampSendChat('/ans ' .. param .. ' Èãðîê íå â ñåòè' .. doptext)
		else
...
sampRegisterChatCommand("sbanip", function(arg)
	if arg:find('(.+) (.+) (.+)') then
        ...
        sampAddChatMessage('/banip [íèê] [÷èñëî] [ïðè÷èíà]', -1)
    end
end)
...
sampRegisterChatCommand('obm', function(param) 
	if #param ~= 0 then
		sampSendChat('/iban ' .. param .. ' 7 Îáìàí/Ðàçâîä')
	else
		sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Âû íå óêàçàëè çíà÷åíèå.')
	end
end)
...
sampRegisterChatCommand('neadekv', function(param) 
	if #param ~= 0 then
		sampSendChat('/iban ' .. param .. ' 3 Íåàäåêâàòíîå ïîâåäåíèå')
	else
		sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Âû íå óêàçàëè çíà÷åíèå.')
	end
end)
