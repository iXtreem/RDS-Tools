...
function main()
	...
	if AdminTools then
		...
		if sampGetCurrentServerAddress() ~= '46.174.52.246' and sampGetCurrentServerAddress() ~= '46.174.49.170' then
			...
			ScriptExport()
	 	else sampAddChatMessage(tag.. '������ ������� ��������. ���������: ������� ' .. cfg.settings.open_tool .. ' ��� /tool', -1) end
	end
						...
						for _, razdel in pairs(basic_command) do
							for name, command in pairs(razdel) do
								if string.sub(getInput, 1,1) == "/" and name:match("/"..string.gsub(getInput, '%p', '')) and not tonumber(string.sub(command, -1))  then
									...
									count = count + 5
									if count > 16 then break break end -- �� ����� 8 ���������
								end
							end
						end
						count = 0
...
for k,v in pairs(basic_command.rmute) do 
	sampRegisterChatCommand(string.sub(k.."f",2), function(param) if #param ~= 0 then sampSendChat(string.gsub(	string.gsub(v, '/rmute', '/muteoff'), '_', param) ) else sampAddChatMessage(tag .. '�� �� ������� ��������.', -1)  end end)
	sampRegisterChatCommand(string.sub(k,2), function(param) if #param ~= 0 then sampSendChat(string.gsub(v, '_', param)) else sampAddChatMessage(tag .. '�� �� ������� ��������.', -1) end end) end
for k,v in pairs(basic_command.jail) do
	sampRegisterChatCommand(string.sub(k.."f",2), function(param) if #param ~= 0 then sampSendChat(string.gsub(	string.gsub(v, '/jail', '/jailoff'), '_', param) ) else sampAddChatMessage(tag .. '�� �� ������� ��������.', -1)  end end)
	sampRegisterChatCommand(string.sub(k,2), function(param) if #param ~= 0 then sampSendChat(string.gsub(v, '_', param)) else sampAddChatMessage(tag .. '�� �� ������� ��������.', -1) end end) end
			...
			if menu == '������� ����' then
				...
				if imgui.BeginPopup('settings_ears') then  
					...
					if imgui.Button(u8'�������� �������',imgui.ImVec2(140,24)) then
						if array.checkbox.check_render_ears.v then
							lua_thread.create(function()
								if #array.ears > 1 then array.ears[#array.ears] = '�������� ��������� ��� ��������� ����� �������.' end 
								sampAddChatMessage(tag .. '��������� ����� ������� ����: Enter', -1)
								sampAddChatMessage(tag .. '�������� ������� �������: Esc',-1)
	...
	if string.sub(command, 1, 1) =='/' and string.sub(command, 1, 2) ~='/a' then
		...
		elseif command:match('ban (%d+) (%d+) .+') or command:match('banoff (%d+) (%d+) .+') or command:match('banakk (%d+) (%d+) .+') then
			if command:match('off') or command:match('akk') then
				...
				sampAddChatMessage(tag .. '/sbanip ��������� ������� ������ � �������� ������ � IP �������. �� �������� ������ � ��� +', -1)
			end
...
function sampev.onServerMessage(color,text) -- ��������� ��������� �� ����
	...
	elseif not AFK then
		if cfg.settings.automute and (text:match("%((%d+)%): (.+)") or text:match("%[(%d+)%]: (.+)")) 			and not (text:match("%[a%-(%d+)%] (%(.+)%) (.+)%[(%d+)%]:") or text:match('������� %[(%d+)%]:') or text:match('������� (.+)%[(%d+)%]: ')) then
			...
			local report = false
			if text:match('������') then report = true end
		...
		elseif text==('%<AC%-WARNING%> {ffffff}(.+)%[(%d+)%]{82b76b} ������������� � ������������� ���%-��������%: {ffffff}Weapon hack %[code%: 015%]%.') and cfg.settings.weapon_hack then
...
function automute(array, oskid, text, nakaz, report)
	...
	if cfg.settings.option_automute == 1 then
		...
		lua_thread.create(function()
			...
			sampAddChatMessage(colorc..'===================={'..color()..'} AutoMute AT '..colorc..'====================', -1)
			sampAddChatMessage('������ ��� �� �������: "' ..nakaz_name ..'"? ������� '..cfg.settings.key_automute ..' ��� �������������',-1)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text) -- ������ � ��������� ���������
	...
	if title == '{ff8587}������������� ������� (������)' and cfg.settings.render_admins then
		...
		lua_thread.create(function()
			...
			for i = 1, #array.admins do -- {FFFFFF}N.E.O.N(0) ({2E8B57}��.�������������{FFFFFF}) | �������: {ff8587}6{FFFFFF} | ��������: {ff8587}0 �� 3{FFFFFF} | ���������: {ff8587}
