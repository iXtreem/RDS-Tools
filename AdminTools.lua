...
script_name 'RDS Tools'  -- Ïðîñüáà íè÷åãî â êîäå íå ìåíÿòü, åñëè ìåíÿåòå - òî íà ñâîé ñòðàõ è ðèñê, ìåíÿ äàæå íå ñïðàøèâàéòå.
...
local mp = import "\\resource\\RDSToolsMP.lua" -- ïîäãðóçêà ïëàãèíà äëÿ ìåðîïðèÿòèé
...
function main() -- îñíîâíîé ñöåíàðèé ñêðèïòà
	...
	update_url = "https://raw.githubusercontent.com/iXtreem/RDS-Tools/main/RDSTools.ini" -- Ññûëêà íà êîíôèã
	update_path = getWorkingDirectory() .. "/RDSTools.ini" -- è òóò òó æå ñàìóþ ññûëêó
	...
	sampRegisterChatCommand('update', function()
		...
		if update_state then
			downloadUrlToFile(script_url, script_path, function(id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Ñêðèïò óñïåøíî îáíîâëåí.')
...
function imgui.OnDrawFrame()
	...
	if main_window_state.v then -- ÊÍÎÏÊÈ ÈÍÒÅÐÔÅÉÑÀ F3
		...
		if menu2[1] then
			...
			imgui.SameLine()
			imgui.SetCursorPosX(160)
			if imgui.Checkbox(u8'Àâòîôîðìû', checked_test4) then
				cfg.settings.slejkaform  = not cfg.settings.slejkaform
				inicfg.save(cfg,directIni)
			end
			if imgui.Checkbox(u8'Àâòîìóò îñê/ìàò', checked_test1) then
			...
			if update_state then
				if imgui.Button(u8'Îáíîâèòü ñêðèïò', imgui.ImVec2(300, 24)) then
					...
					setVirtualKeyDown(13, true)
					setVirtualKeyDown(13, false)
					sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Îáíîâëåíèå çàãðóæàåòñÿ, îæèäàéòå.', -1)
				end
			end
			imgui.Separator()
		...
		if menu2[3] then
			...
			if imgui.Button(u8'Ñîõðàíèòü ïîçèöèþ Recon Menu', imgui.ImVec2(300, 24)) then
				if four_window_state.v then
					...
					sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Çàéäèòå â ñëåæêó âî èçáåæàíèÿ ðàññèõðîíà', -1)
				end
			end
			imgui.SetCursorPosX(10)
			if cfg.settings.acon then
	...
	if tree_window_state.v then -- áûñòðûé îòâåò íà ðåïîðò
		...
		if imgui.Button(u8'Íàêàçàòü', imgui.ImVec2(120, 25)) then
			if tonumber(autorid) then
				...
				sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Äàííûé èãðîê íå â ñåòè. Èñïîëüçóéòå /rmuteoff', -1)
			end
		end
		imgui.SameLine()
	...
	if dopcustomreport_window_state.v then -- äîï äåéñòâèÿ â êàñòîì ðåêîí ìåíþ
		...
		if imgui.Button(u8"Òåëåïîðòèðîâàòü ê ñåáå", imgui.ImVec2(250, 25)) then
			if not sampIsDialogActive() then
				...
				sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Çàêðîéòå äèàëîã.')
			end
		end
	...
	if ansreport_window_state.v then -- ïîìîùü ïîñëå ñëåæêè â ðåêîíå
		...
		if imgui.Button(u8'Íàðóøåíèé íå íàáëþäàþ', imgui.ImVec2(250, 25)) then
			if sampIsPlayerConnected(saveplayerrecon) then
				...
				sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}Èãðîê íå â ñåòè.')
				saveplayerrecon = nil
				ansreport_window_state.v = false
			end
		end
						...
						while d ~= 0 do
							...
							if don == '/' then
								...
								if probid and sampIsPlayerConnected(probid) then
									...
									sampAddChatMessage('{FF0000}RDS Tools: {FFFFFF}ID íå îáíàðóæåí, ëèáî íàõîäèòñÿ âíå ñåòè.', -1)
									break
								end
							end
						end
	...
	if cfg.settings.automute then
		...
		if text:match('(.+)%((.+)%): {(.+)}(.+)') then
			for k,v in pairs(cfg.osk) do
				if text:match(v) and not text:match('%A%A' .. v) and not text:match('ÿ ' .. V) then
			...
			for k,v in pairs(cfg.mat) do
				if text:match(v) and not text:match('%A%A' .. v) then
					...
					if matid then
						sampSendChat('/mute ' .. matid .. ' 300 Íåöåíçóðíàÿ ëåêñèêà')
					end
					return false
				end
			end
...
sampRegisterChatCommand('add_mat', function(param)
    ...
    if not a then
        ...
        cfg.mat[key] = param
        inicfg.save(cfg,directIni)
        sampAddChatMessage('{DDA0DD}[AutoMute]:{F0E68C} Ñëîâî{008000} ' .. param .. ' {F0E68C}óñïåøíî äîáàâëåíî â ñïèñîê ìàòîâ', -1)
        a = nil
    end
end)
...
sampRegisterChatCommand('textform', function(param) 
	...
	inicfg.save(cfg,directIni)
	sampAddChatMessage('{C0C0C0}RDS Tools: {FAEBD7}òåêñò ïðèíÿòèÿ ôîðìû îáíîâëåí', -1)
end)
